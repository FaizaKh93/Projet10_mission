...
def test_generate_embeddings_batch(vector_store):
    ...
    # Chunks factices minimaux : _generate_embeddings() ne lit que la clé "text"
    # (voir vector_store.py:103 : texts_to_embed = [chunk["text"] for chunk in batch_chunks])
    fake_chunks = [
        {"text": "Stephen Curry a marqué 2000 points"},
        {"text": "Nikola Jokić a pris 800 rebonds"},
    ]
